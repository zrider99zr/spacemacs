...
(it can be enabled essentially for testing purpose). "
  ;; load the lock file depending on which git branch is checked-out
  (configuration-layer/load-lock-file)
  (let ((continue-p t))
    ;; when the stable lock file has been loaded
    (when (string-equal configuration-layer--lock-filename
                        configuration-layer-lock-file-stable)
      (when
          (or
        ...
        (setq continue-p configuration-layer//stable-elpa-install)))
    continue-p))
...
(defun configuration-layer//stable-elpa-prompt-install-locked-version ()
  ...
  (y-or-n-p
   (format (concat "A new stabe ELPA version is available: %s"
      ...
      (unwind-protect
          (setq continue-p
                (progn
                  ...
                  (configuration-layer//stable-elpa-clean)
